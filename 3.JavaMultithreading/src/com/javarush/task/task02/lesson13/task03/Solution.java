...
public class Solution {
    ...
    public static boolean checkTelNumber(String telNumber) {
        ...
        if (telNumber.matches("\\+(\\d+)?(\\(\\d{3}\\))?(-?\\d+){0,2}-?\\d+") && telNumber.replaceAll("\\D", "").length() == 12)
            return true;

        if (telNumber.matches("(\\d+)(\\(\\d{3}\\))?(-?\\d+)?-?\\d+") && telNumber.replaceAll("\\D", "").length() == 10)
            return true;

        if (telNumber.matches("(\\(\\d{3}\\))(-?\\d+)?-?\\d+") && telNumber.replaceAll("\\D", "").length() == 10)
            return true;
    ...
    public static void main(String[] args) {
        ...
        String info = countError == 0 ? "Test complete successfully" : "Count Error = " + countError;
